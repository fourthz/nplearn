...
The packages required for this vignette are nplearn, gtools, and coin.
...
what does the symmetry_test function do with these data? I subtracted A from 
B in this example, so I need to switch around to an upper-sided test.
...
If you are thinking that we have now wandered into the realm of strong conditions for valid inference, you are partly correct, but only partly. There is no reason for pessimism. When we randomize members of matched pairs to conditions and the null hypothesis is true, the difference scores for these 
pairs will be symmetrical, even if the original scores are not.
